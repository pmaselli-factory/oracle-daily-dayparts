...
async function seleccionarTiposDeOrden(page) {
  ...
  // El click en el centro del ul abre Y cierra el dropdown
  // Necesitamos abrirlo y mantenerlo abierto — usar el oj-listbox-input directamente
  // El input de búsqueda dentro del dropdown es el que realmente lo activa
  
  // Click en el ul para abrir
  await page.mouse.click(ulCoords.x, ulCoords.y);
  await sleep(2000);

  // Si el dropdown no está abierto, intentar con el input
  let isOpen = await page.evaluate(() => {
    const d = document.querySelector('.oj-listbox-drop');
    if (!d) return false;
    return d.style.display !== 'none' && d.getBoundingClientRect().height > 0;
  });
  console.log(`  🔍 Dropdown estado: ${isOpen}`);

  if (!isOpen) {
    // Intentar con el oj-listbox-input directamente via JS
    await page.evaluate(() => {
      const ul = document.querySelector('#order_type_advance_selectMany_report-filter-order-type .oj-select-choices');
      if (ul) {
        // Disparar mousedown en el ul que es lo que Oracle usa para abrir
        ul.dispatchEvent(new MouseEvent('mousedown', { bubbles: true, cancelable: true }));
        ul.dispatchEvent(new MouseEvent('mouseup', { bubbles: true, cancelable: true }));
      }
    });
    await sleep(1500);
    isOpen = await page.evaluate(() => {
      const d = document.querySelector('.oj-listbox-drop');
      return d ? d.style.display !== 'none' && d.getBoundingClientRect().height > 0 : false;
    });
    console.log(`  🔍 Dropdown estado (2do intento): ${isOpen}`);
  }

  // Buscar Local — con o sin dropdown abierto intentar encontrarlo
  const localCoords = await page.evaluate(() => {
    const dropdown = document.querySelector('.oj-listbox-drop');
    const allLis = dropdown ? Array.from(dropdown.querySelectorAll('li'))
      .filter(el => el.getBoundingClientRect().height > 0) : [];
    console.log('LIs visibles:', allLis.map(l => l.innerText?.trim()).join('|'));
    const li = allLis.find(el => el.innerText?.trim() === 'Local');
    if (!li) return { found: false, count: allLis.length };
    const r = li.getBoundingClientRect();
    return { found: true, x: Math.round(r.x + r.width/2), y: Math.round(r.y + r.height/2) };
  });
  console.log(`  🔍 localCoords:`, JSON.stringify(localCoords));

  if (localCoords?.found) {
    await page.mouse.click(localCoords.x, localCoords.y);
    console.log(`  📌 Local: (${localCoords.x}, ${localCoords.y})`);
    await sleep(800);
  } else {
    console.log(`  ⚠️ No encontró Local (${localCoords?.count} LIs visibles)`);
  }
